...
public function reviewListOffice()
{
    $applications = Application::with([
            ...
            'employmentHistories'])
        ->where('job_type', 'office')
        ->whereIn('status', ['review-list', 'interview']) // atau status khusus review
        ->latest()
        ->get();
    ...
    public function store(Request $request)
{
    ...
    try {
        ...
        $applicationData = [
            ...
            'email' => $validated['email'] ?? 'tidak-ada@example.com', // Default email untuk produksi
        ...
        if ($job->recruitment_type === 'production') {
            ...
            // Tambahkan validasi khusus untuk SMA/SMK
    ...
    public function destroy(Application $application)
    {
        ...
        try {
            ...
            // Hapus file utama
            $filesToDelete = [
                'photo' => public_path($application->photo),
                'cv' => public_path($application->cv),
                'cover_letter' => public_path($application->cover_letter),
                'merged_pdf' => storage_path('app/public/merged_' . $application->id . '.pdf')
            ];

            foreach ($filesToDelete as $type => $filePath) {
                if ($filePath && file_exists($filePath)) {
                    if (!unlink($filePath)) {
                        Log::warning("Gagal menghapus file {$type} untuk aplikasi ID: {$application->id}");
                    }
                }
            }
