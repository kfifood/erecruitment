@extends('layouts.app')
@section('title', 'Kandidat Diterima')
@section('content')
<div class="container">
    <div class="card">
        <div class="card-header">
            <h3 class="mb-3 text-center" style="font-size:1.5rem; font-weight:700; color: var(--bluedark-color);">Kandidat Diterima (Hired)</h3>
        </div>
        <div class="card-body">
            @if($scores->isEmpty())
                <div class="alert alert-info">
                    <i class="fas fa-info-circle me-2"></i> Tidak ada kandidat yang diterima
                </div>
            @else
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @php
                                        // Ambil hanya 1 kelebihan dan 1 kekurangan tertinggi
                                ...
                                <td>
                                    @if($score->decision)
                                        <span class="badge bg-{{ $score->decision === 'hired' ? 'success' : 'danger' }}">
                                            {{ $score->decision === 'hired' ? 'Diterima' : 'Tidak Diterima' }}
                                        </span>
                                    @else
                                ...
                                <td>
                                    ...
                                    @endif
                                    <a href="{{ route('interview-scores.show', $score->id) }}" target="_blank" class="btn btn-sm btn-info">
...
function confirmSendResult(scoreId, decision) {
    if (confirm(`Anda akan mengirim hasil DITERIMA ke kandidat. Lanjutkan?`)) {
